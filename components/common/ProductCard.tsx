...
const ProductCard: React.FC<{ product: Product }> = ({ product }) => {
  ...
  return (
    ...
    >
      {/* Product Image Stage */}
      <Link to={`/products/${product.id}`} className="relative aspect-[4/5] overflow-hidden bg-gray-50 rounded-[1.8rem] md:rounded-[2.2rem]">
        <img
        ...
        {/* Wishlist Marker */}
        <button className="absolute top-4 right-4 md:top-8 md:right-8 p-2 md:p-3 text-white/40 hover:text-red-500 transition-all opacity-0 group-hover:opacity-100">
          <Heart className="h-4 w-4 md:h-5 md:w-5 fill-transparent hover:fill-current" />
        </button>
      </Link>
